            ...
            ? data.map(p => `
                <tr>
                  ...
                  <td class="text-center" style="white-space:nowrap">
                    ...
                    <button class="btn btn-outline btn-sm" onclick="printNotaPO(${p.id})" title="Cetak Nota">🖨️</button>
...
function productOptions() {
    return '<option value="">-- Pilih Produk --</option>' +
        productsList.map(p => {
            ...
            const label = ` [${formatNum(bc)} ${u}/${bu}]`;
            return `<option value="${p.id}" data-cost="${p.cost_price}" data-buy-content="${bc}" data-unit="${u}" data-buy-unit="${bu}">${p.code} – ${p.name}${label}</option>`;
        }).join('');
}
...
function addItemRow() {
    ...
    tr.innerHTML = `
      ...
      <td class="text-right" style="font-size:12px;color:#666"><span class="stock-info">-</span></td>
...
function setCost(sel) {
    ...
    const bc   = opt ? (parseFloat(opt.dataset.buyContent) || 1) : 1;
    const unit = opt ? (opt.dataset.unit || 'kg') : 'kg';
    const buyUnit = opt ? (opt.dataset.buyUnit || unit) : unit;
    const tr = sel.closest('tr');
    tr.querySelectorAll('input')[1].value = cost;
    // Show stock info
    const qty = parseFloat(String(tr.querySelectorAll('input')[0].value).replace(/,/g,'.')) || 1;
    tr.querySelector('.stock-info').textContent = `+${formatNum(qty * bc)} ${unit}`;
    calcTotal();
}
...
function calcTotal() {
    ...
    document.querySelectorAll('#itemRows tr').forEach(tr => {
        ...
        if (sel && sel.value) {
            const opt = sel.options[sel.selectedIndex];
            const bc = parseFloat(opt.dataset.buyContent) || 1;
            const unit = opt.dataset.unit || 'kg';
            const stockInfo = tr.querySelector('.stock-info');
            if (stockInfo) stockInfo.textContent = `+${formatNum(qty * bc)} ${unit}`;
        }
    });
...
async function savePO() {
    ...
    const supplier_address = document.getElementById('fSupplierAddress').value.trim();
    ...
    try {
        await apiFetch('/api/purchases', { method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({ po_number, date, supplier_name: supplier, supplier_address, status, notes, items, warehouse_id: warehouse_id ? parseInt(warehouse_id) : null }) });
...
async function viewPO(id) {
    try {
        ...
        const itemRows = data.items.map(i => {
            ...
            const bc = parseFloat(prod?.buy_content) || 1;
            const unit = prod?.unit || 'kg';
            const bu = prod?.buy_unit || unit;
            const stockAdd = i.quantity * bc;
            return `<tr>
          <td>${i.product_code} – ${i.product_name}</td>
          <td class="text-right">${formatNum(i.quantity)} ${bu}</td>
          <td class="text-right">${formatRupiah(i.unit_price)}</td>
          <td class="text-right">${formatRupiah(i.subtotal)}</td>
          <td class="text-right" style="color:#43a047;font-weight:600">+${formatNum(stockAdd)} ${unit}</td>
        </tr>`;
        }).join('');
        document.getElementById('detailContent').innerHTML = `
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;font-size:13px">
            ...
            <div><strong>Alamat:</strong> ${data.supplier_address || '-'}</div>
            <div><strong>Status:</strong> ${statusBadge(data.status)}</div>
          </div>
          <table style="width:100%;border-collapse:collapse;font-size:13px">
            <thead><tr style="background:#f5f7fa"><th style="padding:8px">Produk</th><th style="padding:8px;text-align:right">Qty</th><th style="padding:8px;text-align:right">Harga</th><th style="padding:8px;text-align:right">Subtotal</th><th style="padding:8px;text-align:right">Stok Masuk</th></tr></thead>
            <tbody>${itemRows}</tbody>
          </table>
          <div style="text-align:right;margin-top:12px;font-size:16px;font-weight:700">Total: ${formatRupiah(data.total)}</div>
          <div style="text-align:center;margin-top:16px">
            <button class="btn btn-primary btn-sm" onclick="printNotaPO(${data.id})">🖨️ Cetak Nota</button>
          </div>`;
        document.getElementById('detailModal').classList.add('show');
    } catch (e) { showToast(e.message, 'error'); }
}

async function printNotaPO(id) {
    try {
        const { data } = await apiFetch(`/api/purchases/${id}`);
        ...
        if (notaImgUrl) {
            headerHtml = '<div class="kop">'
                ...
                + '<div>Kepada&nbsp;&nbsp;: <b>' + (data.supplier_name || '-') + '</b></div>'
                + '<div>Alamat&nbsp;&nbsp;: ' + (data.supplier_address || '') + '</div>'
                + '</div></div>';
        } else {
            headerHtml = '<div class="kop">'
...
window.printNotaPO = printNotaPO;
